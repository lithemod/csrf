...
function csrf(array $options = [])
{
     ...
     * @param callable $next
     */
    return function (\Lithe\Http\Request $req, \Lithe\Http\Response $res, $next) use ($name, $expire, $checkBody, $bodyMethods, $regenerate) {
        $csrf = new class($req, $name, $expire, $checkBody, $regenerate)
        {
             ...
             */
            public function __construct(\Lithe\Http\Request $req, string $name, $expire, $checkBody, $regenerate)
            {
                $this->name = $name;
                $this->req = $req;
                $this->expire = $expire;
        ...
        if ($checkBody && in_array($req->method, $bodyMethods)) {
            ...
            if (!$csrf->verifyToken($token, true)) {
                throw new \Lithe\Exceptions\Http\HttpException(419, 'PAGE EXPIRED');
            }
        }
